...
SELECT cvw.year,
	...
	LAG (cvw.year) OVER (
		...
		ORDER BY cvw.year) AS prev_year,
	LAG (round (avg (cvw.avg_wage) :: NUMERIC, 2)) OVER (
		PARTITION BY cvw.industry_branch
		ORDER BY cvw.year) AS prev_year_avg_wage,	
	cvw.industry_branch
...
SELECT cvf.year,
	...
	LAG (cvf.year) OVER (
		...
		ORDER BY cvf.year) AS prev_year,
	LAG (round (avg (cvf.food_price_per_quarter) :: NUMERIC, 2)) OVER (
		PARTITION BY cvf.food
		ORDER BY cvf.year) AS prev_year_avg_food_price,	
	cvf.food
...
average_food_prices_per_year AS (SELECT cvf.year,
	round (avg (cvf.food_price_per_quarter) :: NUMERIC, 2) AS avg_food_price,
	LAG (cvf.year) OVER (
		PARTITION BY cvf.food
		ORDER BY cvf.year) AS prev_year,
...
WHERE (afpy.food ILIKE 'chléb%' OR afpy.food ILIKE 'mléko%')
...
SELECT afpy.food,
	round (avg (ppdy.percentual_price_difference), 2) percentual_price_difference_2006_18
FROM average_food_prices_per_year afpy
...
SELECT afpynf.year,
	...
	CASE 
        WHEN ppdy.percentual_price_difference - pwdy.percentual_wage_difference > 10 
...
SELECT pgr.year,
	...
	CASE WHEN pgr.percentual_GDP_raise > 3 THEN 'significant_raise'
		WHEN pgr.percentual_GDP_raise < 0 THEN 'drop'
		ELSE 'normal' 
		END AS significant_GDP_raise,
	pwdy.percentual_wage_difference,
	CASE WHEN pwdy.percentual_wage_difference > 5 THEN 'significant_raise'
		WHEN pwdy.percentual_wage_difference < 0 THEN 'drop'
		ELSE 'normal' 
		END AS significant_wages_raise,
	ppdy.percentual_price_difference,
	CASE WHEN ppdy.percentual_price_difference> 5 THEN 'significant_raise'
